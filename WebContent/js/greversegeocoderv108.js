 ...
 */
 ...
 /* Change Log 10/09/08
  *
  * - The bounding box used for the GClientGeocoder.setViewport method was too narrowly choosen.
  *   The new default value is 0.1 instead of 0.01 degrees. This can be set by changing the 
  *   GReverseGeocoder.bounds value, before doing a request.
  *
  */

 /* Change Log 3/09/08
  *
  * - Self consturcting start address instead of relying on the placemark.address property to
  *   use with the house number support.
  *
  */
...
GReverseGeocoder.prototype.handleError = function(){
  GEvent.trigger(this, "error", this.lastpoint);
}
...
GReverseGeocoder.prototype.processDirection = function(){
  ...
  if (this.gdirections.getNumRoutes() != 0 ){
    ...
    var sw = new GLatLng(Number(this.lastpoint.lat()) - this.bounds, Number(this.lastpoint.lng()) - this.bounds);
    var ne = new GLatLng(Number(this.lastpoint.lat()) + this.bounds, Number(this.lastpoint.lng()) + this.bounds);
